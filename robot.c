...
int in_left = 0;
int in_right = 0;
int prev_right_sensor = 0;

void robotAutoMotorMove(struct Robot * robot, int front_centre_sensor, int left_sensor, int right_sensor) {
// start moving
    if (start_move == 1) {
        // need to near the wall on the right
        printf("%d", front_centre_sensor);
        printf("%d", right_sensor);
        printf("%d", left_sensor);
        if (front_centre_sensor > 0 && right_sensor > 0) {
            // turn left
        ...
        in_left = 2;
    } else if (right_sensor >= 3 || right_sensor > prev_right_sensor && prev_right_sensor > 2) {
        // turn left
        robot->direction = LEFT;
        in_left = 0;
        if (right_sensor == 4) {
    ...
    } else if (right_sensor < 2 || right_sensor < prev_right_sensor && right_sensor > 0) {
        // turn right
